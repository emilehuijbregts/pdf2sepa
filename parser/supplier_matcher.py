...
def _collect_match_signals(invoice: dict) -> list[str]:
    ...
    if str(invoice.get("vat_number") or "").strip():
        signals.append("vat")
    if invoice.get("payment_term_days") is not None:
        signals.append("payment_term")
    return signals
...
def match_suppliers(invoices: list[dict], db: SupplierDB) -> list[dict]:
    ...
    for invoice in invoices:
        ...
        if supplier:
            ...
            # Alleen bij confirmed; bij needs_review mogen onzekere DB-hits geen
            # leverancier/IBAN/klantcode overschrijven.
            if status == "confirmed":
                ...
                if sup_iban:
                    ...
                    all_ibans_raw = invoice.get("all_ibans") if isinstance(invoice.get("all_ibans"), list) else []
                    sup_iban_clean = db._clean_iban(sup_iban)
                    inv_iban_clean = db._clean_iban(inv_iban)
                    if inv_iban and inv_iban_clean != sup_iban_clean:
                        invoice_copy["iban_mismatch"] = True
                        # region agent log
                        _agent_debug_log(
                            "supplier_matcher.py:match_suppliers:iban_mismatch_marked",
                            "Factuur gemarkeerd met iban_mismatch",
                            {
                                "source_file": str(invoice.get("source_file") or ""),
                                "supplier_selected": str(supplier.get("name") or ""),
                                "supplier_hint": str(invoice.get("supplier_hint") or ""),
                                "invoice_customer_number": str(invoice.get("customer_number") or ""),
                                "invoice_vat_number": str(invoice.get("vat_number") or ""),
                                "invoice_kvk_number": str(invoice.get("kvk_number") or ""),
                                "invoice_email_domain": str(invoice.get("email_domain") or ""),
                                "inv_iban": inv_iban,
                                "sup_iban": sup_iban,
                                "all_ibans": [str(x or "") for x in all_ibans_raw],
                ...
                candidate = _clean_supplier_candidate(candidate)
                invoice_copy["supplier_name"] = candidate or None
                invoice_copy["supplier_match_source"] = "needs_review_untrusted_db"
            ...
            if inv_iban and is_plausible_iban(inv_iban):
                candidate = _sanitize_supplier_hint(inv_hint)
                source_kind = "supplier_hint"
                if _is_poor_supplier_candidate(candidate):
                    candidate = email_name
                    source_kind = "email_domain"
                invoice_copy["supplier_name"] = _clean_supplier_candidate(candidate) or "Onbekende leverancier"
                invoice_copy["supplier_match_source"] = f"new_from_iban:{source_kind}"
                ...
                if _is_poor_supplier_candidate(candidate):
                    candidate = email_name
                candidate = _clean_supplier_candidate(candidate)
                if candidate and len(parsed_signals) >= 2:
                    invoice_copy["supplier_name"] = candidate
                    invoice_copy["supplier_match_source"] = "new_from_signals"
                    invoice_copy["match_status"] = "new"
                else:
