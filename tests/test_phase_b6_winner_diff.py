"""Phase B6 winner-diff vs committed Phase A.1 snapshot."""
...
REPORT_PATH = Path(__file__).resolve().parents[1] / "reports" / "phase_b6_winner_diff.json"
PHASE_LABEL = "B6"


def test_phase_b6_winner_diff_report(observability_bundle: dict[str, Any]) -> None:
    ...
    report = {
        "phase": PHASE_LABEL,
        "note": (
            f"Phase {PHASE_LABEL}: parser field scalars are written through "
            "resolve_field + apply_resolved_field_result; production winners must match Phase A.1."
        ),
    ...
    assert production_changes == [], (
        "Production winners drifted in Phase B6:\n"
        + "\n".join(
