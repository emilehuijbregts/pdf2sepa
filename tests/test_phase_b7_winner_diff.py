"""Phase B7 winner- and status-diff vs committed Phase A.1 snapshot."""
...
REPORT_PATH = Path(__file__).resolve().parents[1] / "reports" / "phase_b7_winner_diff.json"
PHASE_LABEL = "B7"


def test_phase_b7_winner_diff_report(observability_bundle: dict[str, Any]) -> None:
    ...
    report = {
        "phase": PHASE_LABEL,
        "note": (
            f"Phase {PHASE_LABEL}: field status is assigned only in field_resolver._build_result; "
            "payment_engine reads status without re-promotion; no post-resolve status mutation."
        ),
        "summary": {
        ...
        "status_change_log": status_changes,
        "post_resolve_status_mutation": False,
        "status_authority": "field_resolver._build_result",
        "payment_engine_status_mode": "read_only",
    }
    ...
    assert production_changes == [], (
        "Production winners drifted in Phase B7:\n"
        + "\n".join(
    ...
    assert status_changes == [], (
        "Production field status drifted in Phase B7:\n"
        + "\n".join(
            f"  {c['pdf']} :: {c['field']}: {c['old_status']!r} -> {c['new_status']!r}"
            for c in status_changes[:40]
        )
    )
