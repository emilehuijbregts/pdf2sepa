...
class TestBasicGeneration:
    def test_generates_file(self, tmp_export_dir, debtor, exec_dt):
        path = generate_xml([_payment()], debtor, exec_dt, output_dir=tmp_export_dir)
...
class TestTransactionCounts:
    ...
    def test_multiple_payments(self, tmp_export_dir, debtor, exec_dt):
        payments = [_payment(invoice_number=f"INV{i}") for i in range(5)]
        path = generate_xml(payments, debtor, exec_dt, output_dir=tmp_export_dir)
        root = etree.parse(path).getroot()
        txs = root.findall(f".//{qn('CdtTrfTxInf')}")
        assert len(txs) == 5

    def test_nb_of_txs_correct(self, tmp_export_dir, debtor, exec_dt):
        payments = [_payment(invoice_number="A"), _payment(invoice_number="B")]
        path = generate_xml(payments, debtor, exec_dt, output_dir=tmp_export_dir)
        root = etree.parse(path).getroot()
...
class TestCtrlSum:
    def test_ctrl_sum_matches(self, tmp_export_dir, debtor, exec_dt):
        payments = [
            _payment(amount=10.10, invoice_number="P1"),
            _payment(amount=20.20, invoice_number="P2"),
            _payment(amount=30.30, invoice_number="P3"),
        ]
        path = generate_xml(payments, debtor, exec_dt, output_dir=tmp_export_dir)
        root = etree.parse(path).getroot()
...
class TestSpecialCharacters:
    def test_unicode_supplier_name(self, tmp_export_dir, debtor, exec_dt):
        p = _payment(supplier_name="Müller & Zöhne GmbH")
        path = generate_xml([p], debtor, exec_dt, output_dir=tmp_export_dir)
        root = etree.parse(path).getroot()
...
class TestErrorCases:
    def test_empty_payments_raises(self, tmp_export_dir, debtor, exec_dt):
        with pytest.raises(ValueError):
            generate_xml([], debtor, exec_dt, output_dir=tmp_export_dir)
