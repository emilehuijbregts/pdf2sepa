"""Phase B3 winner-diff vs committed Phase A.1 snapshot (B2 production baseline)."""
...
REPORT_PATH = Path(__file__).resolve().parents[1] / "reports" / "phase_b3_winner_diff.json"
PHASE_LABEL = "B3"


def test_phase_b3_winner_diff_report(observability_bundle: dict[str, Any]) -> None:
                ...
                production_changes.append(
                    {
                        ...
                        "reason": "production_pipeline_drift_vs_b2_baseline_snapshot",
                    }
                )
    ...
    report = {
        "phase": PHASE_LABEL,
        "note": (
            f"Phase {PHASE_LABEL}: parser amount path delegates to rank_key(parse); "
            "production winners must match Phase A.1/B2 snapshot."
        ),
    ...
    assert production_changes == [], (
        "Production winners drifted vs B2 baseline:\n"
        + "\n".join(
