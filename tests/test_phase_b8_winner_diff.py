"""Phase B8 winner-diff vs committed Phase A.1 snapshot."""
...
REPORT_PATH = Path(__file__).resolve().parents[1] / "reports" / "phase_b8_winner_diff.json"
PHASE_LABEL = "B8"
DEAD_CODE_REMOVED = [
    "_generic_is_strong",
    "_generic_is_weak",
...
def test_phase_b8_winner_diff_report(observability_bundle: dict[str, Any]) -> None:
    ...
    report = {
        "phase": PHASE_LABEL,
        "note": (
            f"Phase {PHASE_LABEL}: G5 dead-code cleanup removed unused resolver/candidate helpers; "
            "production winners must match Phase A.1."
        ),
    ...
    assert production_changes == [], (
        "Production winners drifted in Phase B8:\n"
        + "\n".join(
